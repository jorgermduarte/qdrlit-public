...
class QuantumLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # q_out = [0.234, -0.567, 0.789, 0.123]
        q_out = self.qnn(projected)
        
        # boosted Shape: (batch_size, 4) → (batch_size, 256)
        boosted = self.quantum_boost(q_out)

        # Output: residual = [0.234, -0.567, 1.234, ... ] 
        residual = self.residual_proj(x)  # Shape: (batch_size, 256) → (batch_size, 256)
        
        print(f"q_out residual norm: {torch.norm(residual)},q_out quantum norm: {torch.norm(boosted)}")
        
        return boosted + residual  # Shape: (batch_size, 256)
